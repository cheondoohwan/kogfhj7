...
public class CaptchaController {
    ...
    public String cap02ok(HttpServletRequest req){
        ...
        // captcha 문자열의 정확성 여부를 위해
        // 비밀키와 captcha 문자열을 포함함 검증 url작성
        String secretKey = "6LfX8fYdAAAAABGSyoRnsTrdi9B_gIXaYUk_yzac";
        String verifyURL = "https://www.google.com/recaptcha/api/siteverify";
        String params = "secret=" + secretKey + "&response=" + gCaptRes;

        try {
            String jsonData = ""; // 결과저장용 변수 선언

            URL url = new URL(verifyURL);
            HttpURLConnection huconn = (HttpURLConnection) url.openConnection();
            huconn.setRequestMethod("POST"); //요청방식 시정
